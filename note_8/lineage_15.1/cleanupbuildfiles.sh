...
SAUCE=~/android/lineage/out/target/product/greatlte
DEVICE=greatlte
...
if [ -e $SAUCE/lineage-15.1-*-UNOFFICIAL-$DEVICE.zip ]; then
	rm $SAUCE/lineage-15.1-*-UNOFFICIAL-$DEVICE.zip
fi
...
if [ -e $SAUCE/lineage-15.1-*-UNOFFICIAL-$DEVICE.zip.md5sum ]; then
	rm $SAUCE/lineage-15.1-*-UNOFFICIAL-$DEVICE.zip.md5sum
fi
