...
SAUCE=~/android/aosp60/out/target/product/espresso
DEVICE=espresso
...
if [ -e $SAUCE/ua_espresso-6.0.0-*.zip ]; then
	rm $SAUCE/ua_espresso-6.0.0-*.zip
fi
...
if [ -e $SAUCE/ua_espresso-6.0.0-*.zip.md5sum ]; then
	rm $SAUCE/ua_espresso-6.0.0-*.zip.md5sum
fi
