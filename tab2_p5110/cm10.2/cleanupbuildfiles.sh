...
SAUCE=~/android/lineage/out/target/product/p5110
DEVICE=p5110
...
if [ -e $SAUCE/cm-10.2-*-UNOFFICIAL-$DEVICE.zip ]; then
	rm $SAUCE/cm-10.2-*-UNOFFICIAL-$DEVICE.zip
fi

if [ -e $SAUCE/cm_$DEVICE-ota-*.zip ]; then
	rm $SAUCE/cm_$DEVICE-ota-*.zip
fi

if [ -e $SAUCE/cm-10.2-*-UNOFFICIAL-$DEVICE.zip.md5sum ]; then
	rm $SAUCE/cm-10.2-*-UNOFFICIAL-$DEVICE.zip.md5sum
fi
