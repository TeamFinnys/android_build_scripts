...
SAUCE=~/android/lineage-13/out/target/product/p5110
DEVICE=p5110
...
if [ -e $SAUCE/OmniROM-4.4.4-*-UNOFFICIAL-$DEVICE.zip ]; then
	rm $SAUCE/OmniROM-4.4.4-*-UNOFFICIAL-$DEVICE.zip
fi

if [ -e $SAUCE/OmniROM-4.4.4_$DEVICE-ota-*.zip ]; then
	rm $SAUCE/OmniROM-4.4.4_$DEVICE-ota-*.zip
fi

if [ -e $SAUCE/lineageOS-13-*-UNOFFICIAL-$DEVICE.zip.md5sum ]; then
	rm $SAUCE/lineageOS-13-*-UNOFFICIAL-$DEVICE.zip.md5sum
fi
